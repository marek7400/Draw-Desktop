...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QStyle, QDialogButtonBox # Added QDialogButtonBox
)
from PySide6.QtGui import (
    ...
    QMouseEvent, QShowEvent, QDesktopServices # Added QShowEvent, QDesktopServices
)

# Add QSettings, QRect explicitly if not already fully imported
from PySide6.QtCore import Qt, QPointF, QRectF, QSizeF, QTimer, Signal, Slot, QEvent, QRect, QSettings, QPoint, QAbstractNativeEventFilter, QUrl # Added QUrl for links
...
class GlobalHotkeyFilter(QAbstractNativeEventFilter):
    def __init__(self, callback):
        ...
        # Prevent rapid double triggers for toggle hotkeys
        self.last_hotkey_time = 0
        self.min_interval = 0.3 # Minimum interval between hotkey presses (seconds)
        ...
        if event_type == "windows_generic_MSG":
            try:
                ...
                if msg.message == 0x0312:  # WM_HOTKEY
                    ...
                    # Check time interval for toggle hotkeys
                    # Apply interval check only to the toggle hotkey
                    if hotkey_id == HOTKEY_ID_DRAW: # Changed to use new ID name
                        if current_time - self.last_hotkey_time < self.min_interval:
                            print(f"[FILTER] Hotkey ID {hotkey_id} ignored (too soon)")
                            return True, 0 # Consume the event
                        self.last_hotkey_time = current_time

                    print(f"[FILTER] Hotkey pressed! ID: {hotkey_id}")
                    self.callback(hotkey_id)
                    return True, 0 # Indicate the hotkey was handled here
            except Exception as e:
                print(f"Error in nativeEventFilter: {e}")
        return False, 0 # Let other filters process the event
...
if _IS_WINDOWS:
    try:
        ...
        MOD_SHIFT = 0x0004
        MOD_WIN = 0x0008
        # Virtual Key Codes (Check these if keys don't work)
        VK_OEM_5 = 0xDC  # Usually backslash '\' on US keyboards
        ...
        MOD_ALT = MOD_CONTROL = MOD_SHIFT = MOD_WIN = VK_OEM_5 = VK_SNAPSHOT = 0 # Dummy values
        _IS_WINDOWS = False
else:
    user32 = gdi32 = None
    CreateRectRgn = CombineRgn = DeleteObject = SetWindowRgn = lambda *args: None
...
# --- Global Hotkey IDs (Changed for Item 3) ---
# NOTE: RegisterHotKey doesn't reliably distinguish Left/Right modifiers.
# We'll use standard modifiers and update descriptions.
HOTKEY_ID_DRAW = 1             # Toggle Drawing Mode (Ctrl + \) -> Was Ctrl + \
HOTKEY_ID_EDIT = 2             # Enter EDIT Mode (Alt + \) -> Was Alt + \
HOTKEY_ID_BOARD = 3            # Enter BOARD Mode (Ctrl + Alt + \) -> Was Ctrl + Alt + \
HOTKEY_ID_SHIFT_PRINTSCREEN = 4 # Screenshot (Shift + PrintScreen) - Item 2
...
class DesktopOverlayRgn(QWidget):
    ...
    edit_mode_changed = Signal(bool) # Signal for EDIT mode status - Item 3
    board_mode_changed = Signal(bool) # Signal for BOARD mode status - Item 3
    ...
    def _register_global_hotkey(self):
        if not _IS_WINDOWS or not VK_OEM_5: # Check if backslash VK code is available
             print("Skipping global hotkey registration (Not Windows or VK_OEM_5 undefined)")
             return False
        ...
        # --- Define Hotkeys (Item 3 change) ---
        # NOTE: Cannot reliably distinguish Left/Right Alt/Ctrl with RegisterHotKey.
        # Using standard modifiers. Descriptions updated below.
        hotkeys_to_register = [
            {"id": HOTKEY_ID_DRAW, "vk": VK_OEM_5, "mod": MOD_CONTROL, "name": "Ctrl+\\"},
            {"id": HOTKEY_ID_EDIT, "vk": VK_OEM_5, "mod": MOD_ALT, "name": "Alt+\\"},
            {"id": HOTKEY_ID_BOARD, "vk": VK_OEM_5, "mod": MOD_CONTROL | MOD_ALT, "name": "Ctrl+Alt+\\"},
            {"id": HOTKEY_ID_SHIFT_PRINTSCREEN, "vk": VK_SNAPSHOT, "mod": MOD_SHIFT, "name": "Shift+PrintScreen"}, # Item 2
        ]
        ...
        for hotkey in hotkeys_to_register:
            ...
            if RegisterHotKey(self.hwnd, hotkey_id, modifiers | MOD_NOREPEAT, vk_key): # Added MOD_NOREPEAT
    ...
    def _unregister_global_hotkey(self):
        if _IS_WINDOWS and self.hwnd:
            # Use the NEW hotkey IDs
            hotkey_ids_to_unregister = [HOTKEY_ID_DRAW, HOTKEY_ID_EDIT, HOTKEY_ID_BOARD, HOTKEY_ID_SHIFT_PRINTSCREEN] # Item 2 Added Screenshot HK
            for hotkey_id in hotkey_ids_to_unregister:
                print(f"Unregistering global hotkey ID {hotkey_id} for HWND {self.hwnd}.")
                if not UnregisterHotKey(self.hwnd, hotkey_id):
                    err = ctypes.GetLastError()
                    # Error 1413: ERROR_HOTKEY_NOT_REGISTERED (ignore this specific error)
                    if err != 1413:
                        print(f"  Failed to unregister hotkey ID {hotkey_id}. Error code: {err}")

    # Removed nativeEvent override here - handled by GlobalHotkeyFilter installed by MainApplication
    ...
    def __init__(self):
        ...
        self.edit_mode = False # Specific flag for EDIT mode (subset of board_mode) - Item 3
        self.shapes = []
        self.undo_stack = []
        self.redo_stack = []
    ...
    def paintEvent(self, event):
        try:
            ...
            # --- Background ---
            if self.board_mode or self.edit_mode: # Use flags - Item 3 logic
                # Use board_background_color directly (includes alpha)
                draw_color = self.board_background_color
                # Override for EDIT mode transparency check
                if self.edit_mode and draw_color.alpha() >= 5:
                     draw_color = QColor(0, 0, 0, 1) # Ensure Edit mode is near-transparent
            ...
            elif self.drawing_mode: # Normal draw mode
            ...
            if self.drawing_mode:
                # Determine if any indicators should be shown based on board/edit mode and settings
                show_any_indicator = ((self.board_mode or self.edit_mode) and self.show_indicators_in_board_mode) or \
                                     (not self.board_mode and not self.edit_mode and self.show_tool_text)
                ...
                if show_any_indicator:
                    ...
                    bg_is_dark = False
                    if self.board_mode or self.edit_mode: # Use flags
                        effective_bg_color = self.board_background_color
                        if self.edit_mode: effective_bg_color = QColor(0,0,0,1) # Assume dark for edit overlay
                    ...
                    painter.save()
                    indicator_color = self.current_pen_color_board if (self.board_mode or self.edit_mode) else self.current_pen_color
                    ...
                    # Draw BOARD/EDIT Indicator (Top-Right) only if in board/edit mode
                    if self.board_mode or self.edit_mode:
                        # Indicate EDIT mode specifically
                        board_text = "EDIT MODE" if self.edit_mode else "BOARD" # Use edit_mode flag
    ...
    def draw_shape(self, painter, shape, is_selected=False, is_preview=False):
        ...
        # Item 4 Change: Use shape.filled attribute directly
        if shape.filled and shape.type in ['rect', 'ellipse', 'triangle', 'polygon']:
    ...
    @Slot(bool)
    def set_drawing_mode(self, enabled, called_from_hotkey=False): # Added flag - Item 2
        if self.drawing_mode == enabled:
            print(f"set_drawing_mode called but state is already {enabled}")
            # Ensure button state is correct even if mode doesn't change
            self.drawing_mode_changed.emit(self.drawing_mode)
            return

        # Prevent exiting drawing mode if cancelling board/edit mode exit fails
        if not enabled and (self.board_mode or self.edit_mode): # Check both flags
            # Try exiting the specific mode first
            if not self.exit_board_mode(ask_save=False, configure=False): # Auto-save now, configure later
                if self.control_panel:
                    # Ensure button state reflects reality if exit was cancelled
                    self.control_panel.update_draw_button_state(True)
                    self.control_panel.update_edit_button_style_active(self.edit_mode)
                    self.control_panel.update_board_button_style_active(self.board_mode and not self.edit_mode)
                return # Cancelled exit

        # Reset board/edit flags if drawing is being disabled
        if not enabled:
            self.board_mode = False
            self.edit_mode = False
            self.board_mode_changed.emit(False) # Item 3
            self.edit_mode_changed.emit(False) # Item 3
        ...
        # Reapply non-board settings when enabling drawing mode (if not in board/edit mode)
        if enabled and not self.board_mode and not self.edit_mode and self.control_panel:
            ...
            print(f"Reapplied NORMAL draw settings: dim_background={self.dim_background}, show_tool_text={self.show_tool_text}")

        # Configure mode (potentially delayed if called from hotkey) - Item 2 Fix
        if called_from_hotkey:
            print("Delaying configure_mode due to hotkey trigger")
            QTimer.singleShot(50, self._configure_mode) # Short delay
        else:
            self._configure_mode() # Configure immediately otherwise

        self.drawing_mode_changed.emit(self.drawing_mode) # Signal change *after* setting state
        # Avoid processEvents here, rely on Qt's event loop
    ...
    def set_board_pen_color(self, color):
        """Sets the pen color for BOARD/EDIT drawing mode."""
        if isinstance(color, QColor) and color.isValid():
            if color != self.current_pen_color_board:
                self.current_pen_color_board = color
                print(f"BOARD/EDIT Pen color set to: {color.name(QColor.HexArgb)}")
    ...
    def set_dim_background(self, dim_enabled):
        ...
        if self.dim_background != dim_enabled:
            ...
            if not self.board_mode and not self.edit_mode and self.drawing_mode:
    ...
    def set_show_tool_text(self, show):
        ...
        if self.show_tool_text != show:
            ...
            # Update show_indicators_in_board_mode if currently in board/edit mode
            if self.board_mode or self.edit_mode:
    ...
    def _enter_board_or_edit_mode(self, edit_mode=False):
        ...
        # Set flags BEFORE enabling drawing mode
        self.board_mode = True # Generic board flag
        self.edit_mode = edit_mode # Specific EDIT flag
        ...
        # Make sure drawing mode is enabled (calls _configure_mode eventually)
        ...
        # Emit specific mode signals - Item 3
        self.board_mode_changed.emit(self.board_mode and not self.edit_mode)
    ...
    def exit_board_mode(self, ask_save=False, configure=True): # Changed ask_save default to False
        ...
        if not self.board_mode and not self.edit_mode: return True # Already exited
        ...
        was_board = self.board_mode
        was_edit = self.edit_mode
        self.board_mode = False # Reset flags
        self.edit_mode = False
        ...
        # Emit signals that modes have ended - Item 3
        if was_board: self.board_mode_changed.emit(False)
        if was_edit: self.edit_mode_changed.emit(False)

        # Exit drawing mode completely after exiting board mode
        if configure:
             # This will call _configure_mode and emit drawing_mode_changed(False)
    ...
    def _configure_mode(self):
        ...
        print(f"_configure_mode called. drawing_mode={self.drawing_mode}, board_mode={self.board_mode}, edit_mode={self.edit_mode}") # Debug
        ...
        if self.drawing_mode:
            ...
            if self.edit_mode: # EDIT mode is always near-transparent
                is_background_opaque = False
                set_translucent_bg_attr = True
                print("Configure Mode: EDIT mode -> Translucent")
            elif self.board_mode: # BOARD mode opacity depends on BG alpha
                is_background_opaque = (self.board_background_color.alpha() == 255)
                set_translucent_bg_attr = not is_background_opaque
                print(f"Configure Mode: BOARD mode -> Opaque={is_background_opaque}, Translucent={set_translucent_bg_attr}")
            else: # Normal drawing mode depends on dimming
                is_background_opaque = self.dim_background
                set_translucent_bg_attr = not is_background_opaque
                print(f"Configure Mode: NORMAL mode -> Opaque={is_background_opaque}, Translucent={set_translucent_bg_attr}")
            ...
            # QTimer.singleShot(50, lambda: self.setFocus(Qt.ActiveWindowFocusReason) if self.drawing_mode else None) # Keep delay for safety
        ...
        if not self.isVisible(): self.show()
        self.update()
        # QApplication.processEvents() # Avoid explicit processEvents if possible
        print("_configure_mode: Configuration finished.")

        # --- Update Control Panel Color Button Visuals ---
        if self.control_panel:
            # Update color styles first
            ...
            # Then update mode button active styles (this uses the logic to determine if green should be applied)
            self.control_panel.update_draw_button_style(self.drawing_mode and not self.edit_mode and not self.board_mode) # Only active if ONLY drawing
            self.control_panel.update_edit_button_style_active(self.edit_mode)
            self.control_panel.update_board_button_style_active(self.board_mode and not self.edit_mode) # BOARD only if not EDIT
        # --- ---
    ...
    def save_state(self, action_type, shapes_involved=None, previous_geometries=None, indices=None, all_shapes_before=None):
        ...
        try:
            state = {
                'action': action_type,
                'involved_shapes_before_action': involved_before if action_type not in ['draw', 'paste', 'load_join'] else None, # Store shapes *before* action for paste/load_join
                'involved_shapes_after_action': involved_after if action_type == 'draw' else None,
                # previous_geometries holds specific *properties* before the change OR paste/load count
                ...
                'load_join_count': previous_geometries if action_type == 'load_join' else None, # Reuse prev_geometries for load_join count
            ...
            # Added load_join case: Save shapes before the join operation
            elif action_type == 'load_join':
                 state['involved_shapes_before_action'] = deepcopy(shapes_involved) # Use shapes_involved passed from the action call
    ...
    def undo(self):
        ...
        try:
            ...
            redo_state = {
                 ...
                 'load_join_count': state_to_undo.get('load_join_count'), # Added
                            ...
                            if props_before is not None:
                                try:
                                     ...
                                     if action in ['move', 'resize', 'scale']:
                                         ...
                                         # Check if props_before is already an enum or needs conversion
                                         if isinstance(props_before, Qt.PenStyle): target_shape.line_style = props_before
                                         elif isinstance(props_before, int):
                                             try: target_shape.line_style = Qt.PenStyle(props_before)
                                             except ValueError: valid_restore = False; print(f"Invalid PenStyle value for undo: {props_before}")
                                         else: valid_restore = False
    ...
    def redo(self):
        ...
        try:
            ...
            undo_state = {
                 ...
                 'load_join_count': state_to_redo.get('load_join_count'), # Added
            ...
            if action == 'edit_text':
                 ...
                 if indices and 0 <= indices[0] < len(self.shapes):
                      ...
                      if edited_shape.type == 'text' and edited_shape.text_properties:
                          ...
                          if not self.board_mode and not self.edit_mode: # Check mode flags
                          ...
                          elif self.board_mode or self.edit_mode:
    ...
    def mousePressEvent(self, event: QMouseEvent):
        ...
        if event.button() == Qt.LeftButton:
            ...
            if top_shape: # Clicked on existing shape
                ...
                # Fill check (Alt+F key) handled by keypress
                if not self.board_mode and not self.edit_mode and top_shape_idx != -1:
                ...
                use_board_props = self.board_mode or self.edit_mode # Check flags
                ...
                # Item 4 Change: Fill depends on control panel setting AND shape type
                ...
                if self.current_tool in ['rect', 'ellipse']:
                    ...
                    # Fill only applies to these types based on checkbox
                    is_fillable = self.current_tool in ['rect', 'ellipse']
                    actual_fill = filled if is_fillable else False
                    self.current_drawing_shape = Shape(self.current_tool, geom, pen_color, filled=actual_fill, alpha=alpha, line_thickness=thickness, line_style=line_style )
                    ...
                    self.current_drawing_shape = Shape(self.current_tool, [pos, pos], pen_color, filled=False, alpha=alpha, line_thickness=thickness, line_style=line_style, arrow_head_size=(arrow_size if self.current_tool == 'arrow' else None) )
                     ...
                     # Fill only applies based on checkbox
                    is_fillable = self.current_tool in ['triangle']
                    actual_fill = filled if is_fillable else False
                    self.current_drawing_shape = Shape('triangle', [pos, pos, pos], pen_color, filled=actual_fill, alpha=alpha, line_thickness=thickness, line_style=line_style )
    ...
    def finish_drawing_poly_line(self):
        ...
        if len(self.polygon_points) >= min_points:
            ...
            # Determine properties based on mode
            use_board_props = self.board_mode or self.edit_mode
            pen_color = self.current_pen_color_board if use_board_props else self.current_pen_color
            alpha = self.current_alpha
            thickness = self.current_line_thickness
            line_style = self.current_line_style
            # Item 4 Change: Fill depends on checkbox AND shape type
            is_fillable = tool == 'polygon'
            filled = is_fillable and (self.control_panel.fill_check.isChecked() if self.control_panel else True)
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self.current_drawing_shape and self.current_tool not in ['text', 'polygon', 'line_point']:
                ...
                if valid_final_shape:
                    ...
                    # --- Auto-exit drawing mode ---
                    if not self.board_mode and not self.edit_mode: # Use flags
            ...
            if self.current_tool == 'text' and self.drag_start_pos:
                ...
                initial_props_for_dialog = deepcopy(self.board_default_text_properties if (self.board_mode or self.edit_mode) else self.default_text_properties)
                initial_props_for_dialog['text'] = '' # Start empty
                # Use appropriate color based on mode
                pen_color = self.current_pen_color_board if (self.board_mode or self.edit_mode) else self.current_pen_color
            ...
            if top_shape:
                if top_shape.type == 'text':
                    ...
                    current_props = deepcopy(top_shape.text_properties) if top_shape.text_properties else \
                                    deepcopy(self.board_default_text_properties if (self.board_mode or self.edit_mode) else self.default_text_properties)
    ...
    def _show_color_picker_for_selection(self, key_name):
        """Helper function to show color picker for selected shapes."""
        if not self.selected_shapes:
            print(f"Numpad '{key_name}' pressed, but no shapes selected.")
            return True # Indicate event handled

        print(f"Numpad '{key_name}' detected. Opening color picker for {len(self.selected_shapes)} selected shapes.")
        is_board_or_edit = self.board_mode or self.edit_mode # Use flags
        ...
        if color.isValid():
            ...
            for shape in self.selected_shapes:
                try:
                    ...
                    if is_board_or_edit:
                        new_shape_color.setAlpha(shape.color.alpha())
                    elif shape.type == 'text': # Also preserve for text in normal mode
                         new_shape_color.setAlpha(shape.color.alpha())
            ...
            print("Color dialog cancelled or color invalid.")
        return True # Indicate event handled


    def keyPressEvent(self, event: QKeyEvent):
        # This handles keys ONLY when the overlay window has focus.
        # Global hotkeys are handled by the filter installed in MainApplication.
        ...
        print(f"KeyPress (Overlay Focus): Key={key} ({hex(key)}), Text='{event.text()}', Modifiers={modifiers}, Keypad={keypad_modifier}, Drawing={self.drawing_mode}") # Debug print
        ...
        if key == Qt.Key_Escape:
            ...
            if (self.board_mode or self.edit_mode) and (current_time - self.last_esc_press_time < double_press_interval):
        ...
        # --- Global Hotkeys (\) are handled by the filter ---
        # --- Global Screenshot hotkey is handled by the filter (Item 2) ---

        # --- Numpad Period/Delete for Color Picker --- Item 1 / Item 6 Refactor
        if (key == Qt.Key_Delete or key == Qt.Key_Period) and keypad_modifier:
            if self._show_color_picker_for_selection('.'):
                 event.accept(); return

        # --- Numpad Asterisk for Color Picker --- Item 6
        if key == Qt.Key_Asterisk and keypad_modifier:
            if self._show_color_picker_for_selection('*'):
                 event.accept(); return
        ...
        if self.board_mode or self.edit_mode: # Use flags
            ...
            # BOARD: Pen/Shape Color (Num 0-9, no modifiers)
                ...
                if self.selected_shapes:
                    print(f"BOARD/EDIT: Applying color shortcut {key} to selected: {target_color.name()}")
                    ...
                    print(f"BOARD/EDIT: Applying color shortcut {key} to current pen: {target_color.name()}")
        ...
        elif not self.board_mode and not self.edit_mode and not ctrl_pressed and not alt_pressed and not shift_pressed and key in self.color_shortcuts:
        ...
        # --- Deletion ---
        # Numpad Del/Period handled above by color picker check
        is_numpad_del_or_period = (key == Qt.Key_Delete or key == Qt.Key_Period) and keypad_modifier
        is_numpad_asterisk = key == Qt.Key_Asterisk and keypad_modifier

        if key == Qt.Key_Delete or key == Qt.Key_Backspace:
            if is_numpad_del_or_period or is_numpad_asterisk:
                print(f"Numpad {key} keypress ignored for shape deletion (handled by color picker).") # Debug
        ...
        # --- Item 4: Fill Toggle Shortcut ---
        if key == Qt.Key_F and alt_pressed and not ctrl_pressed and not shift_pressed:
            if self.control_panel:
                current_state = self.control_panel.fill_check.isChecked()
                self.control_panel.fill_check.setChecked(not current_state)
                print(f"Toggled Fill Shapes via Alt+F to: {not current_state}")
                # update_fill_state slot handles applying to selection if uncommented there
                event.accept(); return

        # --- Item 6/5: Screenshot with Shift+PrtSc (Handled Globally Now - Item 2) ---
        # if key == Qt.Key_Print and shift_pressed:
        #     print("Shift+PrintScreen key detected (overlay focus).")
    ...
    # --- Screenshot Method --- Item 2 (Called by Global Hotkey Now)
    def capture_screenshot(self):
        try:
            # Optionally hide the overlay/panel before capture
            # self.hide(); self.control_panel.hide()
            # QTimer.singleShot(100, self._do_capture_screenshot) # Delay slightly

            # Simpler: Capture immediately
            self._do_capture_screenshot()

        except Exception as e:
            print(f"Error preparing screenshot: {e}")
            traceback.print_exc()
            QMessageBox.critical(self, "Screenshot Error", f"An unexpected error occurred during screenshot preparation:\n{e}")
            # Ensure widgets are shown again if they were hidden
            # if not self.isVisible(): self.show()
            # if self.control_panel and not self.control_panel.isVisible(): self.control_panel.show()

    def _do_capture_screenshot(self):
        """Actual screenshot capture and save logic."""
        try:
            screen = QGuiApplication.primaryScreen()
            if not screen:
                print("Error: Could not get primary screen.")
                QMessageBox.warning(self, "Screenshot Error", "Could not access primary screen.")
                return

            # Grab the entire virtual desktop if possible, otherwise primary screen
            desktop_wid = 0 # Passing 0 often grabs the whole screen/virtual desktop
            pixmap = screen.grabWindow(desktop_wid)
            if pixmap.isNull(): # Fallback if desktop grab fails
                print("Whole screen grab failed, grabbing primary screen geometry.")
                geom = screen.geometry()
                pixmap = screen.grabWindow(0, geom.x(), geom.y(), geom.width(), geom.height())

            if pixmap.isNull():
                print("Error: Failed to capture screenshot.")
                QMessageBox.warning(self, "Screenshot Error", "Failed to capture screen content.")
                return
            ...
            filename, selected_filter = QFileDialog.getSaveFileName(
                self, # Parent window
                "Save Screenshot As",
                "screenshot.jpg", # Default filename
                "JPEG Image (*.jpg *.jpeg);;PNG Image (*.png)" # Default to JPG
            )

            if filename:
                # Determine format and quality based on filter/extension
                file_format = None
                quality = -1 # Default Qt quality
                low_filename = filename.lower()
                if ('png' in selected_filter.lower() or low_filename.endswith('.png')):
                    file_format = 'png'
                    quality = -1 # PNG quality is handled differently (compression level)
                elif ('jp' in selected_filter.lower() or low_filename.endswith('.jpg') or low_filename.endswith('.jpeg')):
                    file_format = 'jpg'
                    quality = 95 # Good quality JPEG
                else: # Default to JPG if filter/extension is unclear
                    file_format = 'jpg'
                    quality = 95
                    if not (low_filename.endswith('.jpg') or low_filename.endswith('.jpeg')):
                        filename += ".jpg" # Append extension if missing

                if pixmap.save(filename, file_format, quality):
                    print(f"Screenshot saved to: {filename} (Format: {file_format}, Quality: {quality})")
                    # Use a non-modal message box or just print to console if called from hotkey
                    # QMessageBox.information(self, "Screenshot Saved", f"Screenshot saved successfully to:\n{filename}")
                else:
                    print(f"Error: Failed to save screenshot to {filename}.")
                    QMessageBox.warning(self, "Save Error", f"Could not save screenshot to:\n{filename}")
            else:
            ...
            traceback.print_exc()
            QMessageBox.critical(self, "Screenshot Error", f"An unexpected error occurred during screenshot:\n{e}")
        # finally:
            # Ensure widgets are shown again if they were hidden
            # if not self.isVisible(): self.show()
            # if self.control_panel and not self.control_panel.isVisible(): self.control_panel.show()
    ...
    def show_text_dialog(self, pos_or_origin, shape_to_edit=None, initial_props=None, initial_rect=None):
        ...
        if shape_to_edit: # Editing existing
            ...
            if dialog.exec():
                ...
                if not self.board_mode and not self.edit_mode: # Check flags
                     ...
                     print("Updated BOARD/EDIT text defaults (applied to shape, not signaling panel).")
                # --- End Update Defaults ---
    ...
    def load_scene(self, filename, join=False): # Added join parameter
        ...
        loaded_count = 0 # Keep track of shapes loaded from THIS file
        try:
            with open(filename, 'r', encoding='utf-8') as f: scene_data = json.load(f)
            if not isinstance(scene_data, list):
                 print(f"Error: Loaded data is not a list. Aborting load for file {filename}.")
                 return 0 # Return 0 shapes loaded

            if not join:
                print("Replacing current scene.")
                # The 'load' state is saved *before* calling this function
                ...
                print(f"Joining loaded scene from {filename} with current scene.")
                # Selection clearing and saving state is handled *before* calling for join=True
            ...
            for i, shape_data in enumerate(scene_data):
                 if isinstance(shape_data, dict):
                      ...
                      else: print(f"Warning: Failed to load shape {i} from dict in {filename}: {shape_data}")
                 else: print(f"Warning: Invalid shape data format (not a dict) at index {i} in {filename}: {shape_data}")

            if loaded_shapes:
                 self.shapes.extend(loaded_shapes)
                 loaded_count = len(loaded_shapes)

            self._configure_mode(); self.update()
            print(f"Finished loading {filename}. Added {loaded_count} shapes. Total shapes now: {len(self.shapes)}.")
            return loaded_count # Return number of shapes added from this file
             ...
             print(f"Error loading scene from {filename}: {e}"); traceback.print_exc()
             QMessageBox.warning(self, "Load Error", f"An unexpected error occurred loading the scene from:\n{filename}\n\n{e}")
             return 0
...
class AuthorInfoDialog(QDialog):
    def __init__(self, parent=None):
        ...
        info_label = QLabel()
        # Enable text selection and make link clickable
        info_label.setTextInteractionFlags(Qt.TextSelectableByMouse | Qt.TextSelectableByKeyboard | Qt.LinksAccessibleByMouse)
        info_label.setOpenExternalLinks(True)
        info_label.setTextFormat(Qt.RichText) # Important for link rendering
        info_label.setText("""
        ...
        """)
        # Adjust label size policy to fit content
        info_label.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Minimum)
        info_label.setWordWrap(True) # Not strictly needed for this short text but good practice
...
class ControlPanel(QDockWidget):
    def __init__(self, overlay):
        ...
        self.fill_check = QCheckBox("Fill Shapes"); self.fill_check.setToolTip("Fill Rects, Ellipses, Triangles, Polygons (Toggle: Alt+F)") # Item 4 tooltip
        ...
        indicator_layout = QHBoxLayout() # Layout for indicator checkbox + info button
        self.tool_text_check = QCheckBox("Show Indicators"); self.tool_text_check.setToolTip("Show tool name and mode indicators")
        indicator_layout.addWidget(self.tool_text_check)

        # --- Item 3: Info Button ---
        self.info_button = QPushButton("?")
        self.info_button.setFixedSize(self.tool_text_check.sizeHint().height(), self.tool_text_check.sizeHint().height()) # Match checkbox size
        self.info_button.setToolTip("Show Author Info")
        self.info_button.clicked.connect(self.show_info_dialog)
        indicator_layout.addWidget(self.info_button)
        ...
        # Create buttons (Update Text/Tooltips for Item 3 hotkeys)
        # Note: Actual hotkeys registered might differ if Left/Right cannot be distinguished.
        # Tooltips reflect the intended, but potentially simplified, registration.
        self.drawing_button = QPushButton("DRAW (Ctrl+\\)"); self.drawing_button.setCheckable(True); self.drawing_button.setToolTip("Toggle Drawing Mode (Ctrl+\\)")
        self.edit_button = QPushButton("EDIT (Alt+\\)"); self.edit_button.setToolTip("Enter Edit Mode (Board with Transparent BG) (Alt+\\)"); self.edit_button.clicked.connect(self.overlay.enter_edit_mode)
        self.board_button = QPushButton("BOARD (Ctrl+Alt+\\)"); self.board_button.setToolTip("Enter Whiteboard Mode (Ctrl+Alt+\\)"); self.board_button.clicked.connect(self.overlay.enter_board_mode)
        ...
        # Item 5: Updated tooltip for Load&Join
        load_join_button = QPushButton("Load&Join"); load_join_button.setToolTip("Load Scene(s) (Append, Multiple Files OK)"); load_join_button.clicked.connect(self.load_and_join_scene_action)
        ...
        action_buttons = [
            ...
            redo_button,
            shortcuts_button,
            self.board_button, # BOARD (Ctrl+Alt+\)
            self.edit_button,  # EDIT (Alt+\)
            self.drawing_button, # DRAW (Ctrl+\) last
        ]
        # *** END Button Order ***
        ...
        # --- Style Sheets for Active Modes - Item 3 ---
        self.original_button_style = self.edit_button.styleSheet() # Get default style
        self.active_mode_style = "background-color: lightgreen; border: 1px solid darkgreen;"
        # ---

        # --- Connect Signals ---
        self.drawing_button.toggled.connect(self.handle_draw_mode_toggle) # Keep this for direct clicks on DRAW
        self.overlay.drawing_mode_changed.connect(self.update_draw_button_state)
        self.overlay.edit_mode_changed.connect(self.update_edit_button_style_active) # Item 3
        self.overlay.board_mode_changed.connect(self.update_board_button_style_active) # Item 3
        ...
        # Item 4 Change: Connect fill checkbox state change to overlay shape property
        self.fill_check.stateChanged.connect(self.update_fill_state)
    ...
    def mousePressEvent(self, event: QMouseEvent):
        if event.button() == Qt.LeftButton:
            ...
            # Consider slightly larger area for title bar grabbing
            grab_area_height = title_bar_height + 5
            is_on_title_bar_area = event.position().y() <= grab_area_height

            if is_on_title_bar_area:
                # Check if the click is on an actual title bar button (minimize, close)
                # This part is tricky without direct access to title bar elements.
                # A simpler check: assume if it's high up, it might be title bar unless it's an interactive widget.
                ...
                # If high up and not on an interactive widget, assume drag
                ...
                # If click is below title bar area, check if it's on non-interactive background
                child = self.childAt(event.position().toPoint()); is_interactive = False
                if child:
                    current_widget = child
                    while current_widget and current_widget != self.widget():
                        ...
                        if any(current_widget.inherits(t) for t in interactive_types):
                            is_interactive = True; break
                        current_widget = current_widget.parentWidget()
                # If not on an interactive widget, allow dragging from empty space
                if not is_interactive:
                     self._dragging_window = True
                     self._drag_window_offset = event.globalPosition().toPoint() - self.geometry().topLeft()
                     self.setCursor(Qt.SizeAllCursor)
    ...
    # --- Item 3 / Item 4: Info Dialog Slot ---
    @Slot()
    def show_info_dialog(self):
        # Use the custom dialog
        if self.info_dialog is None:
            self.info_dialog = AuthorInfoDialog(self)
    ...
    def restore_settings(self):
        ...
        self.update_draw_button_style(self.overlay.drawing_mode and not self.overlay.edit_mode and not self.overlay.board_mode)
        self.update_edit_button_style_active(self.overlay.edit_mode)
        self.update_board_button_style_active(self.overlay.board_mode and not self.overlay.edit_mode)
    ...
    def update_draw_color_button_style(self, color):
        ...
        # Item 1 Fix: Only change style if DRAW mode is NOT active (otherwise active style takes precedence)
        # Correction: The logic should be simpler. Just update the color *unless* the green active style is applied.
        # The green style application is handled in update_draw_button_style(is_active).
        if color.isValid():
            bg_color_css = color.name(QColor.HexRgb)
            text_color_css = 'white' if color.lightnessF() < 0.5 else 'black'
            # Only apply color if the button doesn't currently have the green "active" style
            is_active_normal_draw = self.overlay.drawing_mode and not self.overlay.edit_mode and not self.overlay.board_mode
            if not is_active_normal_draw:
                self.drawing_button.setStyleSheet(f"background-color: {bg_color_css}; color: {text_color_css};") # Apply color change
            ...
            is_active_normal_draw = self.overlay.drawing_mode and not self.overlay.edit_mode and not self.overlay.board_mode
            if not is_active_normal_draw:
                self.drawing_button.setStyleSheet(self.original_button_style)
            self.draw_color_button.setStyleSheet(self.original_button_style) # Reset style
            self.draw_color_button.setToolTip("Invalid Draw Color")

    @Slot(QColor)
    def update_edit_color_button_style(self, color):
        """Updates the 'Color (Edit)' button style (non-active state)."""
        if color.isValid():
            bg_color_css = color.name(QColor.HexRgb)
            text_color_css = 'white' if color.lightnessF() < 0.5 else 'black'
            # Only update if not currently in active EDIT or BOARD mode style
            is_edit_active = self.overlay.edit_mode
            is_board_active = self.overlay.board_mode and not self.overlay.edit_mode
            if not is_edit_active: # Check only EDIT as BOARD shares the color button visually
            ...
            # Only reset if not active
             ...
             if not is_edit_active:
                 self.edit_color_button.setStyleSheet(self.original_button_style) # Reset style
             self.edit_color_button.setToolTip("Invalid Edit/Board Color")

    # --- Item 3: Slots to update button styles based on active mode ---
    @Slot(bool)
    def update_edit_button_style_active(self, is_active):
        """Applies/Removes active style for the EDIT button."""
        if is_active:
            self.edit_button.setStyleSheet(self.active_mode_style)
            self.edit_button.setChecked(True) # Visually indicate pressed state
        else:
            self.edit_button.setStyleSheet(self.original_button_style)
            self.edit_button.setChecked(False)
            # Restore color indicator if needed
            # self.update_edit_color_button_style(self.overlay.current_pen_color_board) # Redundant? Style is reset anyway

    @Slot(bool)
    def update_board_button_style_active(self, is_active):
        """Applies/Removes active style for the BOARD button."""
        if is_active:
            self.board_button.setStyleSheet(self.active_mode_style)
            self.board_button.setChecked(True) # Visually indicate pressed state
        else:
            self.board_button.setStyleSheet(self.original_button_style)
            self.board_button.setChecked(False)
            # Restore color indicator if needed
            # self.update_edit_color_button_style(self.overlay.current_pen_color_board) # Redundant? Style is reset anyway
    # --- End Item 3 ---
    ...
    def update_fill_state(self, state):
        ...
        # The actual fill status is checked during shape creation/drawing.
        # Applying to selected shapes is optional and can be done via Alt+F now.
    ...
    # --- Item 5: Load & Join Multiple Files ---
    def load_and_join_scene_action(self):
        filenames, _ = QFileDialog.getOpenFileNames(self, "Load & Join Scene(s)", "", "JSON Files (*.json)")
        if filenames:
            total_loaded = 0
            shapes_before_load = deepcopy(self.overlay.shapes) # Save state *once* before loading all files
            for filename in filenames:
                loaded_count = self.overlay.load_scene(filename, join=True)
                total_loaded += loaded_count
            if total_loaded > 0:
                # Use the number of loaded shapes in the 'previous_geometries' field for undo state
                self.overlay.save_state('load_join', shapes_involved=shapes_before_load, previous_geometries=total_loaded)
                print(f"Joined a total of {total_loaded} shapes from {len(filenames)} file(s).")
            else:
                print("No shapes loaded from selected file(s).")
            # Update is called within load_scene
        else:
            print("Load & Join cancelled.")
    # --- End Item 5 ---
    ...
    def close_all(self):
        ...
        if self.info_dialog and self.info_dialog.isVisible(): self.info_dialog.close() # Close info dialog too
        self.close(); print("Control panel and child windows closed")

    def closeEvent(self, event):
        print("ControlPanel closeEvent triggered.")
        self.save_settings()
        if self.shortcuts_window: self.shortcuts_window.close() # Will trigger save geo
        if self.info_dialog: self.info_dialog.close() # Close info dialog too
        super().closeEvent(event)

    @Slot(bool)
    def handle_draw_mode_toggle(self, checked):
        # This is triggered by the user clicking the DRAW button
        print(f"DRAW button toggled to {checked}")
        # Only change mode if it's not already handled by EDIT/BOARD buttons
        if not self.overlay.edit_mode and not self.overlay.board_mode:
            self.overlay.set_drawing_mode(checked)
        elif checked:
             # If user clicks DRAW while in EDIT/BOARD, exit those modes?
             # Let's explicitly exit EDIT/BOARD if DRAW is clicked while they are active
             print("DRAW button clicked while in EDIT/BOARD mode. Exiting EDIT/BOARD...")
             if self.overlay.exit_board_mode(ask_save=False, configure=False): # Auto-save, don't reconfigure yet
                 # Now that EDIT/BOARD are exited, call set_drawing_mode which will configure correctly
                 self.overlay.set_drawing_mode(True)
             else:
                 # Exit was cancelled, ensure DRAW button goes back to unchecked
    ...
    def update_draw_button_state(self, is_drawing):
        # This is triggered by the overlay changing drawing_mode state
        # Update the DRAW button's visual state (checked and style)
        print(f"update_draw_button_state received: is_drawing={is_drawing}")
        # Prevent signal emission loop when updating button state programmatically
        self.drawing_button.blockSignals(True)
        self.drawing_button.setChecked(is_drawing)
        self.drawing_button.blockSignals(False)
        # Only apply active style if *only* normal drawing is active (not EDIT/BOARD)
        self.update_draw_button_style(is_drawing and not self.overlay.edit_mode and not self.overlay.board_mode)

    def update_draw_button_style(self, is_active_normal_draw):
        """Applies/Removes active style for the DRAW button (only if not in EDIT/BOARD)."""
        if is_active_normal_draw:
            self.drawing_button.setStyleSheet(self.active_mode_style)
            print("[Style Update] DRAW button set to ACTIVE style.") # Debug
        else:
            self.drawing_button.setStyleSheet(self.original_button_style)
            print("[Style Update] DRAW button set to ORIGINAL style.") # Debug
        # Ensure check state matches overlay state (redundant with update_draw_button_state? Check caller)
        # self.drawing_button.setChecked(self.overlay.drawing_mode) # Already handled by update_draw_button_state
...
class ShortcutsWindow(QDialog):
    ...
    def __init__(self, parent=None, settings=None):
        ...
        # Updated Shortcut descriptions for Item 3
        # Note: Left/Right modifiers cannot be guaranteed, using standard names.
        self.shortcuts_data = {
            "General": [
                ("Ctrl+\\", "Toggle Drawing Mode (Global Hotkey)"),
                ("Alt+\\", "Enter EDIT Mode (Global Hotkey)"),
                ("Ctrl+Alt+\\", "Enter BOARD Mode (Global Hotkey)"),
                ("Esc", "Exit Normal Draw Mode / Cancel Draw/Action"),
                ("Esc (x2 in BOARD/EDIT)", "Exit BOARD/EDIT mode (Confirm Clear)"),
                ("Ctrl+Home", "Toggle Control Panel Visibility"),
                ("Ctrl+K", "Toggle This Shortcuts Window"),
            ],
            "Tools": [
             ...
             "File & Scene": [
                ...
                ("Load&Join Button", "Load Scene(s) (Append, Multiple Files OK)"), # Item 5
                ("Shift+C", "Clear Scene"),
                ("EDIT Button / Alt+\\", "Enter Edit Mode (Transparent BG)"),
                ("BOARD Button / Ctrl+Alt+\\", "Enter Whiteboard Mode"),
                ("Shift+PrtSc", "Save Screenshot (JPG/PNG) (Global Hotkey)"), # Item 2
            ],
            "Edit & Undo": [
                ("Ctrl+Z", "Undo"), ("Ctrl+Y / Ctrl+Shift+Z", "Redo"),
                ("Ctrl+C", "Copy Selected"), ("Ctrl+V", "Paste at Cursor"),
                ("Delete / Backspace", "Delete Selected"),
                ("Numpad '.' / Numpad Del", "Open Color Picker for Selected Shape(s)"),
                ("Numpad *", "Open Color Picker for Selected Shape(s)"), # Item 6
            ...
            "Quick Properties": [ # Simplified section header
                 ("Ctrl+Alt+LMB Click (Shape)", "(NORMAL Mode Only) Apply Current Draw Color"),
                 ("Ctrl+Alt+Shift+LMB Click (Shape)", "(NORMAL Mode Only) Apply Current Draw Alpha"),
                 ("Alt+F", "Toggle Fill Shapes (for new shapes)"), # Item 4
            ],
...
class MainApplication(QApplication):
     def __init__(self, args):
          ...
          # Install global hotkey filter
          self.hotkey_filter = GlobalHotkeyFilter(lambda hotkey_id: self.handle_global_hotkey(hotkey_id))
          self.installNativeEventFilter(self.hotkey_filter)


     def handle_global_hotkey(self, hotkey_id):
         # Check against NEW hotkey IDs (Item 3 change)
         if hotkey_id == HOTKEY_ID_DRAW:
             print("Global Hotkey Ctrl+\\ triggered → toggling drawing mode")
             # Pass flag to indicate source - Item 2 fix
             self.overlay.set_drawing_mode(not self.overlay.drawing_mode, called_from_hotkey=True)
         elif hotkey_id == HOTKEY_ID_EDIT:
             print("Global Hotkey Alt+\\ triggered → entering EDIT mode")
             self.overlay.enter_edit_mode()
         elif hotkey_id == HOTKEY_ID_BOARD:
             print("Global Hotkey Ctrl+Alt+\\ triggered → entering BOARD mode")
             self.overlay.enter_board_mode()
         elif hotkey_id == HOTKEY_ID_SHIFT_PRINTSCREEN: # Item 2
             print("Global Hotkey Shift+PrintScreen triggered → capturing screenshot")
             self.overlay.capture_screenshot()
         else:
     ...
     def exec(self):
         ...
         try:
             ...
             if self.overlay and (self.overlay.board_mode or self.overlay.edit_mode):
...
if __name__ == "__main__":
    ...
    # Filter, Overlay, and Control Panel are created and managed within MainApplication now
